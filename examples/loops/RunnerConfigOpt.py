...
class RunnerConfig:
    ...
    name: str = "loop_experiment_resit" 
    results_output_path: Path = ROOT_DIR / 'experiments_loops_yes' 
    ...
    def create_run_table_model(self) -> RunTableModel:
        ...
        looplist = []
        for i in range(1, 11):
            looplist.append(f"loop{i}_opt_batching")
            looplist.append(f"loop{i}_opt_cacheblocking")
            looplist.append(f"loop{i}_opt_unrolling")
        factor5 = FactorModel("loop_type", looplist)
    ...
    def start_run(self, context: RunnerContext) -> None:
        ...
        self.target = subprocess.Popen(['python3', f'examples/loops/loops_dir/{file_name}'])
